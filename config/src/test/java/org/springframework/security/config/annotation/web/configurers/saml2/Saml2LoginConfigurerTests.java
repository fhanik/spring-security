...
public class Saml2LoginConfigurerTests {
	...
	@Test
	public void saml2Login() throws Exception {
		...
		assertThat(authentication.getAuthorities()).first()
				.isInstanceOf(SimpleGrantedAuthority.class).hasToString("ROLE_USER");
	}


	@EnableWebSecurity
	static class Saml2LoginConfig extends WebSecurityConfigurerAdapter {
	...
	private static ObjectPostProcessor<Object> getPostProcessor() {
		return new ObjectPostProcessor<Object>() {
			@Override
			public <O> O postProcess(O object) {
				if (object instanceof AuthenticationProvider) {
					return (O) getAuthenticationProviderMock();
				}
				return object;
			}
		};
	...
	private static AuthenticationProvider getAuthenticationProviderMock() {
	...
	private static RelyingPartyRegistration getSaml2AuthenticationConfiguration() {
	...
	private static Saml2X509Credential getVerificationCertificate() {
	...
	private static X509Certificate x509Certificate(String source) {
		try {
